...
namespace Chess_Windows_Client
{
	class Pawn : FigureImpl, ChessFigure
	{
		...
		public bool Move(ref ChessGameField.Cell[,] field, Point posFrom, Point posTo)
		{
			ChessGameField.Cell cellTo = field[posTo.X, posTo.Y];
			if (cellTo.figure != null && cellTo.figure.GetOwner() != GetOwner())
			{
			...
			else if (cellTo.figure == null && posFrom.X == posTo.X)
			{
